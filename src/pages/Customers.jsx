...
export default function CustomersDataGrid() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      const [customersRes, regionsRes] = await Promise.allSettled([
        getCustomers(),
        getRegions(),
      ]);
      ...
      if (customersRes.status === "fulfilled") {
        const rawCustomers =
          customersRes.value?.data?.data?.customers ||
      ...
      if (regionsRes.status === "fulfilled") {
        const rawRegions =
          regionsRes.value?.data?.data?.regions ||
          regionsRes.value?.data?.regions ||
          [];
        setRegions(rawRegions);
      } else {
        console.error("❌ Failed to load regions:", regionsRes.reason);
        setError((prev) => prev || "Failed to load regions");
      }
    } catch (err) {
  ...
  const handleCSVUpload = async (e) => {
    ...
    try {
      ...
      console.error("❌ CSV upload failed:", err);
      setError(err.response?.data?.message || "Error uploading CSV file");
    } finally {
      setLoading(false);
    }
  };
